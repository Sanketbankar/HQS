...
// Route handler: /api/actress/:actress/:page?
const handler = async (req, res) => {
    try {
        const { actress } = req.params;
        const page = parseInt(req.params.page) || 1;

        // Build actress URL
        const url =
            page === 1
                ? `${BASE}/actress/${actress}`
                : `${BASE}/actress/${actress}/${page}`;

        const response = await axios.get(url);
        const $ = cheerio.load(response.data);

        const siteTitle = $("header h1").text().trim() || actress;
        const results = {
            currentPage: page,
            availablePages: [],
            totalPages: 1,
        ...
        if (!results.availablePages.includes(page) && page !== 1) {
            return res.status(404).json({
                status: "not_found",
                message: `Page ${page} is not available for category "${actress}".`,
        ...
        $("div.6u").each((i, div6u) => {
            ...
            sections.each((j, section) => {
                ...
                let href = a.attr("href") ? BASE + a.attr("href") : null;
                ...
                if (rawSrc) {
                    if (rawSrc.startsWith("//")) imgSrc = "https:" + rawSrc;
                    else if (rawSrc.startsWith("/")) imgSrc = BASE + rawSrc;
                    else imgSrc = rawSrc;
                }
        ...
        if (results[siteTitle].length === 0) {
            return res.status(404).json({
                status: "not_found",
                message: `No videos found in category "${category}" on page ${page}.`,
            });
        }
...
// Routes
router.get("/:actress", handler); // page 1
router.get("/:actress/:page", handler); // other pages
