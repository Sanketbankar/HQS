...
// Route handler: /api/category/:category/:page?
    ...
    try {
        ...
        const url =
            page === 1
                ...
                : `${BASE}/category/${category}/${page}`;

        const response = await axios.get(url);
        const $ = cheerio.load(response.data);

        const siteTitle = $("header h1").text().trim() || category;
        const results = {
            currentPage: page,
            availablePages: [],
            totalPages: 1,
        ...
        if (!results.availablePages.includes(page) && page !== 1) {
            return res.status(404).json({
                status: "not_found",
                message: `Page ${page} is not available for category "${category}".`,
...
// Routes
router.get("/:category", handler); // page 1
router.get("/:category/:page", handler); // other pages
