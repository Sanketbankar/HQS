...
// /api/search?q=ava addams&page=2
const handler = async (req, res) => {
    try {
        const query = req.query.q;
        if (!query) {
            return res.status(400).json({ status: "error", message: "Missing search query (?q=...)" });
        }

        // Replace spaces with "+" and ensure only one "+"
        const cleanQuery = query.trim().replace(/\s+/g, "+");

        const page = parseInt(req.query.page) || 1;
        const url = page === 1
            ? `https://hqporner.com/?q=${cleanQuery}`
            : `https://hqporner.com/?q=${cleanQuery}&p=${page}`;

        const response = await axios.get(url);
        const $ = cheerio.load(response.data);

        const siteTitle = $("header h1").text().trim() || `Search: ${query}`;
        const results = {
            currentPage: page,
            availablePages: [],
            totalPages: 1,
        ...
        if (results[siteTitle].length === 0) {
            return res.status(404).json({
                status: "not_found",
                message: `No results found for query "${query}" on page ${page}.`,
            });
        }
        ...
        console.error("❌ Search scraping error:", err.message);
        res.status(500).json({ status: "error", message: err.message });
    }
};

// Example: /api/search?q=ava addams&page=2
router.get("/", handler);
